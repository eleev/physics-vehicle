...
//
//  Created by Astemir Eleev on 12/08/2018.
//  Copyright © 2018 Astemir Eleev. All rights reserved.
//

import SpriteKit

class VehicleHumvee: SKNode, VehicleProtocol {
    ...
    required init(position: CGPoint) {
        ...
        chasis = SKSpriteNode(texture: bodyTexture, size: CGSize(width: 400, height: 197.43))
        chasis.zPosition = 8
        ...
        let tireSize = CGSize(width: 94, height: 94)
        
        leftWheel = SKSpriteNode(texture: tireTexture, size: tireSize)
        leftWheel.zPosition = 10
        leftWheel.position = CGPoint(x: (chasis.position.x - chasis.size.width / 2) + 58, y: chasis.position.y - wheelOffsetY)
